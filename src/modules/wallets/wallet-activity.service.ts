...
export async function fetchWalletActivity(
   ...
   const typeFilter =
      type === 'buy' ? 'KEY_BOUGHT' : type === 'sell' ? 'KEY_SOLD' : undefined;
   ...
   const creatorIds = [
      ...new Set(
         rows
            .map((r: { creatorId: string | null }) => r.creatorId)
            .filter(Boolean)
      ),
   ] as string[];
   ...
   const items: WalletActivityItem[] = rows.map(
      (row: {
         id: string;
         type: string;
